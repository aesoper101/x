...
func ValidateHostname(hostname string) (string, error) {
	...
	if err := isValidDomainName(parsedHost); err != nil {
		return "", fmt.Errorf(
			"must either be a valid IP address or domain name: invalid domain name %q, %w",
			hostname,
			err,
		)
	}
	...
	for _, char := range hostname {
		...
		case char == '.':
			...
			if segmentLen > maxSegmentLength {
				return fmt.Errorf(
					"cannot have segments greater than %v characters between periods (.)",
					maxSegmentLength,
				)
			}
			segmentLen = 0
		default:
			return fmt.Errorf(
				"included invalid character %q, must only contain letters, digits, periods (.), hyphens (-), or underscores (_)",
				char,
			)
		}
		previous = char
