...
func (e *Env) extract(key string, value string) (string, interface{}) {
	...
	for _, path := range e.paths {
		...
		if normalized == key {
			...
			case jsonschemaext.String:
				return name, cast.ToString(value)
			case jsonschemaext.Float:
				return name, cast.ToFloat64(value)
			case jsonschemaext.Int:
				return name, cast.ToInt64(value)
			case jsonschemaext.Bool:
				return name, cast.ToBool(value)
			case jsonschemaext.Nil:
				return name, nil
			case jsonschemaext.BoolSlice:
				if !gjson.Valid(value) {
					return name, cast.ToBoolSlice(value)
				}
				fallthrough
			case jsonschemaext.StringSlice:
				if !gjson.Valid(value) {
					return name, toStringSlice(value)
				}
				fallthrough
			case jsonschemaext.IntSlice:
				if !gjson.Valid(value) {
					return name, cast.ToIntSlice(value)
				}
				fallthrough
			case jsonschemaext.FloatSlice:
				if !gjson.Valid(value) {
					return name, toFloatSlice(value)
				}
				fallthrough
			case jsonschemaext.JSON:
